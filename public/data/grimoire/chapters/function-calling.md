...
使用开源 LLM 进行函数调用的方法与 GPT-4 类似，但需要更多的自定义工作。开源模型如 LLaMA、Mistral 等也可以通过微调来实现函数调用能力。
...
- **API 集成**：将自然语言请求转换为 API 调用
- **数据库查询**：将自然语言转换为 SQL 或其他查询语言
- **工具使用**：让 LLM 能够调用外部工具完成复杂任务
